...
class FNO2d(nn.Module):
    ...
    def reset_context_params(self):
        self.context_params = torch.zeros(self.ctx_dim).to('cuda')
        self.context_params.requires_grad = True

    def forward(self, x):
        b, c, size_x, size_y = x.shape 
        ctx = self.ctx_dec(self.context_params).expand(x.shape[0], -1)
...
class CAVIAMLP(nn.Module):
    ...
    def reset_context_params(self):
        self.context_params = torch.zeros(self.num_context_params).to('cuda')
        self.context_params.requires_grad = True
