...
@hydra.main(config_path="../../config/leads/", config_name="train.yaml")
def main(cfg: DictConfig) -> None:
    ...
    # create model folder
    model_dir = Path(os.getenv("WANDB_DIR")) / 'leads' / 'new_ver' / 'pretrain'/ dataset_name / "model"
    ...
    for epoch in range(epochs):
        ...
        for _, data in enumerate(train):
            ...
            env = int(data['env'][0].item())
            t = data['t'][0].to(device)

            outputs = model(states, t, env, epsilon_t)
