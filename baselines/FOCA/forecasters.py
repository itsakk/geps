...
class Derivative(nn.Module):
    ...
    def forward(self, t, y0, ctx):
        return self.model_nn(y0, ctx)

class Forecaster(nn.Module):
    def __init__(self, dataset_name, in_dim, out_dim, ctx_dim, method, options):
        super().__init__()
        self.method = method
    ...
    def forward(self, y, t, ctx, epsilon=0):
        if epsilon < 1e-3:
            epsilon = 0

        if epsilon == 0:
            res = self.int_(partial(self.derivative, ctx = ctx), y0=y[..., 0], t=t, method=self.method, options=self.options)
        else:
            ...
            for i, eval_point in enumerate(eval_points):
                if eval_point:
                    ...
                    res_seg = self.int_(partial(self.derivative, ctx = ctx), y0=y[..., start_i], t=t_seg,
                    ...
                    start_i = end_i
            t_seg = t[start_i:]
            res_seg = self.int_(partial(self.derivative, ctx = ctx), y0=y[..., start_i], t=t_seg, method=self.method,
